...
def number_of_vehicle():
    ...
    if len(all_vehicles) > 1:
        ...
        for distance_of_the_vehicle ,vehicle in sorted_vehicles
            if distance_of_vehicle > 200.0:
                break
            vehicle_type = get_actor_display_name()
            vehicle_data['vehicle_name'] = vehicle_type
            vehicle_data['ditance'] = distance_of__car
            final_vehicle_result.append(vehicle_data)

            for distannce_in meter in final_vehicle_result:
                if distaance_in_meter['distance'] > 3 and distaance_in_meter['distance'] < 6:
